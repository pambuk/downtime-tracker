...
export type ChangeEvent =
  ...
  | { kind: "incident-worsened"; service: ServiceStatus; from: Severity; to: Severity }
  | { kind: "recovered"; service: ServiceStatus; from: Severity };

// Severity ranks used for "worsening" detection.
// `unknown` and `maintenance` are intentionally rank 0 — same as `none` —
// because we don't want to alert on fetch failures or planned maintenance.
 ...
 *  - fire → bigger fire (e.g. minor → major)  => "incident-worsened"
 *  - fire → ok        => "recovered"
 *  - everything else (e.g. critical → major, unknown ↔ none, maintenance flips)
