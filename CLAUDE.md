...
- `src-tauri/` — Rust shell for the desktop build. `tauri.conf.json` controls window/identifier/bundle config. `src/lib.rs` is the entry point where plugins (`tauri-plugin-opener`, `tauri-plugin-notification`, `tauri-plugin-log`) get registered, the macOS activation policy is set to `Accessory` (hides Dock + Cmd-Tab), the `TrayIconBuilder` is configured, and a global window event handler intercepts the close button. The `icons/` folder is the *generated* icon set produced by `npm run tauri -- icon …`; treat its contents as build artifacts. The hand-authored source images live in `src-tauri/icon-source/` — `status-dog-app-icon-1024.png` is the canonical bundle-icon source (used everywhere outside the menu bar: Dock when window is shown, app switcher, notification icon), and `tray-icon/flameTemplate@2x.png` is the *separate* template image used for the menu-bar tray icon (`include_bytes!`'d into `lib.rs` and registered with `.icon_as_template(true)` so macOS adapts it to the current theme). The tray icon is intentionally not the colorful dog — full-color icons look out of place among the system's monochrome template icons in the menu bar. To swap the bundle icon, drop a new high-res PNG into `icon-source/` and run `npm run tauri -- icon src-tauri/icon-source/<file>.png`. To swap the tray icon, replace `icon-source/tray-icon/flameTemplate@2x.png` with another black-on-transparent PNG of similar size.
