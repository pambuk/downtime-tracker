...
- `src-tauri/` — Rust shell for the desktop build. `tauri.conf.json` controls window/identifier/bundle config. `src/lib.rs` is the entry point where plugins (`tauri-plugin-opener`, `tauri-plugin-notification`, `tauri-plugin-log`) get registered, the macOS activation policy is set to `Accessory` (hides Dock + Cmd-Tab), the `TrayIconBuilder` is configured, and a global window event handler intercepts the close button. The `icons/` folder is the *generated* icon set produced by `npm run tauri -- icon …`; treat its contents as build artifacts. The hand-authored source images live in `src-tauri/icon-source/` (`status-dog-app-icon-1024.png` is the canonical source). To swap the icon, drop a new high-res PNG into `icon-source/` and run `npm run tauri -- icon src-tauri/icon-source/<file>.png` to regenerate the bundle icons.
