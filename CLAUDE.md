...
- `src/comments.ts` — sarcastic comments keyed by severity. `pickComment(serviceId, severity)` uses an FNV-1a hash so the chosen line is **stable per (service, severity)** — it doesn't flicker on every re-render but does change when severity changes.
